...
const cats = [
  {
    name: 'Electronics',
    slug: 'electronics',
    description: 'Phones, laptops, audio',
    icon: Zap,
    accent: 'from-cyan-500/10 to-sky-500/5',
    iconBg: 'bg-cyan-500/10',
    iconColor: 'text-[#1daabc]',
    count: '5 products',
  },
  {
    name: 'Fashion',
      ...
      <div className="flex items-end justify-between">
        <div>
          ...
          <p className="mt-2 text-[var(--text-muted)]">
            Four curated departments, thousands of choices.
          </p>
        </div>
        <Link
          href="/products"
        ...
        {cats.map(({ name, slug, description, icon: Icon, accent, iconBg, iconColor }) => (
          <m.li key={slug} variants={revealStaggerItem}>
            <Link
              ...
              className={`group relative flex flex-col gap-4 overflow-hidden rounded-2xl bg-gradient-to-br ${accent} border border-[var(--border-subtle)] p-6 transition-all duration-300 hover:border-[var(--brand-accent)]/40 hover:shadow-lg hover:shadow-[var(--brand-accent)]/5`}
            >
              {/* Icon */}
              <span
                className={`${iconBg} flex h-11 w-11 items-center justify-center rounded-xl transition-transform duration-300 group-hover:scale-110`}
              >
                <Icon
              ...
              <ArrowRight
                aria-hidden="true"
                className={`absolute right-4 top-4 h-4 w-4 ${iconColor} opacity-0 transition-all duration-200 group-hover:translate-x-0.5 group-hover:opacity-100`}
              />
            </Link>
          </m.li>
        ))}
